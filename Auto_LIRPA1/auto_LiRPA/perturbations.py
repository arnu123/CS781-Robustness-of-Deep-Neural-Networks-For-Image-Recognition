...
class PerturbationLpNormWithRegion(PerturbationLpNorm, Perturbation):
    """Perturbation constrained by L_p norm within a square region that can be positioned anywhere in the image."""
    def __init__(self, eps=0, norm=np.inf, square_size=None, x_L=None, x_U=None, eps_min=0):
        super().__init__(eps=eps, norm=norm, x_L=x_L, x_U=x_U, eps_min=eps_min)
        self.square_size = square_size

        if self.norm == 1:
            self.dual_norm = np.inf
        elif self.norm == np.inf:
            self.dual_norm = 1
        else:
            self.dual_norm = self.norm / (self.norm - 1)
        
    def concretize_matrix_conv(self, x, A, sign):
        # Reshape A to (batch_size, output_dim, channels, height, width)
        batch_size = x.shape[0]
        input_shape = x.shape
        ...
        # Reshape A to match the input dimensions
        A = A.view(batch_size, output_dim, channels, height, width)

        # Compute center and diff
        x_L, x_U = self.get_input_bounds(x, A)
        center = (x_U + x_L) / 2.0
        diff = (x_U - x_L) / 2.0

        # Compute A.matmul(center)
        center_flat = center.view(batch_size, -1, 1)  # Shape: (batch_size, input_dim, 1)
        bound_center = torch.matmul(A.view(batch_size, output_dim, -1), center_flat)  # Shape: (batch_size, output_dim, 1)
        bound_center = bound_center.squeeze(-1)  # Shape: (batch_size, output_dim)

        # Prepare A for convolution
        # Take absolute values and raise to the power of dual_norm (q)
        if self.dual_norm == np.inf:
            # For dual_norm = infinity, we need the maximum absolute value in the region
            A_processed = A.abs()
        else:
            A_processed = A.abs().pow(self.dual_norm)  # Shape: (batch_size, output_dim, channels, height, width)

        # Create the convolution kernel
        kernel_size = self.square_size
        kernel = torch.ones((1, 1, kernel_size, kernel_size), device=A.device)

        # Pad A_processed appropriately to ensure the convolution covers all positions
        padding = kernel_size - 1
        A_padded = F.pad(A_processed, (padding, padding, padding, padding), mode='constant', value=0)

        # Reshape A_padded to (batch_size * output_dim * channels, 1, height + padding*2, width + padding*2)
        A_padded = A_padded.view(batch_size * output_dim * channels, 1, height + 2 * padding, width + 2 * padding)

        # Perform convolution to aggregate over the square region
        conv_result = F.conv2d(A_padded, kernel, stride=1)  # Shape: (batch_size * output_dim * channels, 1, conv_height, conv_width)

        # Reshape conv_result back to (batch_size, output_dim, channels, conv_height, conv_width)
        conv_height = conv_result.shape[2]
        conv_width = conv_result.shape[3]
        conv_result = conv_result.view(batch_size, output_dim, channels, conv_height, conv_width)

        # Sum over channels to get the aggregated values per output dimension
        if self.dual_norm == np.inf:
            # For dual_norm = infinity, take the maximum over channels
            agg_result, _ = conv_result.max(dim=2)  # Shape: (batch_size, output_dim, conv_height, conv_width)
        else:
            agg_result = conv_result.sum(dim=2)  # Shape: (batch_size, output_dim, conv_height, conv_width)

        # Compute the norm over the square region
        if self.dual_norm == np.inf:
            # Maximum absolute value already computed
            norms = agg_result
        else:
            # Take the dual norm root
            norms = agg_result.pow(1 / self.dual_norm)  # Shape: (batch_size, output_dim, conv_height, conv_width)

        # Multiply by epsilon to get the maximum possible deviation
        deviation = norms * self.eps  # Shape: (batch_size, output_dim, conv_height, conv_width)

        # Find the maximum deviation over all positions
        deviation = deviation.view(batch_size, output_dim, -1)  # Shape: (batch_size, output_dim, num_positions)
        max_deviation, _ = deviation.max(dim=2)  # Shape: (batch_size, output_dim)

        # Compute final bound
        if sign == -1:
            bound = bound_center - max_deviation
        else:
            bound = bound_center + max_deviation

        return bound
    
    # Modify the `concretize` method to call `concretize_patches` for patch inputs
    def concretize(self, x, A, sign=-1, aux=None):
        """Override concretize to handle patches or square positions."""
        if A is None:
            return None
        
        
        if isinstance(A, eyeC) or isinstance(A, torch.Tensor):
            # Handle dense matrix or identity cases
            # min_bound, max_bound = self.compute_bounds_for_all_positions(x, A)
            return self.concretize_matrix_conv(x, A, sign)
            # Select appropriate bound based on sign
            # if sign == -1:
            #     bound = min_bound
            # else:
            #     bound = max_bound
            ...
            # Handle patch-based cases
            return self.concretize_patches(x, A, sign)
        else:
            raise NotImplementedError("Unknown matrix type for concretization.")

        
    def init(self, x, aux=None, forward=False):
        """Initialize bounds considering perturbations within any square region."""
        self.sparse = False
        batch_size, channels, height, width = x.shape

        # For backward mode, we need to initialize the interval bounds (x_L, x_U)
        if not forward:
            x_L, x_U = self.get_input_bounds(x, None)
            # Since the perturbation can be in any square region, x_L and x_U are the same as x minus and plus the maximum possible perturbation in the region.
            # We can initialize x_L and x_U by computing the minimum and maximum values that x can take within any square region perturbation.

            # Create a mask of ones with the size of the square region
            kernel_size = self.square_size
            mask = torch.ones((1, 1, kernel_size, kernel_size), device=x.device)

            # Pad x appropriately to ensure the convolution covers all positions
            padding = kernel_size - 1
            x_padded = F.pad(x, (padding, padding, padding, padding), mode='constant', value=0)

            # Perform convolution to find the maximum and minimum values
            x_padded = x_padded.view(-1, 1, height + 2 * padding, width + 2 * padding)
            x_conv = F.conv2d(x_padded, mask, stride=1)

            # Reshape back to original batch and channel dimensions
            x_conv = x_conv.view(batch_size, channels, height + padding, width + padding)

            # Since the perturbation can be anywhere, the min and max over all positions are the extremes
            x_min = x_conv.min()
            x_max = x_conv.max()

            # Compute x_L and x_U based on eps
            x_L = x - self.eps
            x_U = x + self.eps

            # Ensure x_L and x_U are within valid ranges (e.g., [0, 1] for image data)
            x_L = torch.clamp(x_L, min=0.0, max=1.0)
            x_U = torch.clamp(x_U, min=0.0, max=1.0)

            # Initialize LinearBound with None for linear coefficients as we don't have lw and uw yet
            return LinearBound(None, None, None, None, x_L, x_U), x, None

        # For forward mode, we need to initialize the linear bounds (lw, lb, uw, ub)
        else:
            dim = x.reshape(batch_size, -1).shape[-1]
            lb = ub = torch.zeros_like(x)

            # Initialize lw and uw as identity matrices scaled by the perturbation magnitude within the square region
            # Since the perturbation can be in any square region, we need to consider the maximum effect
            eye = torch.eye(dim, device=x.device).expand(batch_size, dim, dim)

            # For each position in the square region, the maximum contribution is eps
            lw = eye * (-self.eps)
            uw = eye * self.eps

            # Reshape lw and uw to match x's shape
            lw = lw.view(batch_size, dim, *x.shape[1:])
            uw = uw.view(batch_size, dim, *x.shape[1:])

            # Initialize x_L and x_U
            x_L = x - self.eps
            x_U = x + self.eps
            x_L = torch.clamp(x_L, min=0.0, max=1.0)
            x_U = torch.clamp(x_U, min=0.0, max=1.0)
