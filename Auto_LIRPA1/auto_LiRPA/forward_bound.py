...
from .perturbations import PerturbationLpNorm, PerturbationLpNormWithRegion
    ...
    if concretize:
        ...
        if lw is not None:
            ...
            if lw.shape[1] > 0:
                ...
                ptb = PerturbationLpNormWithRegion(x_L=linear.x_L, x_U=linear.x_U)
    ...
    for i in range(len(roots)):
        if hasattr(roots[i], 'perturbation') and roots[i].perturbation is not None:
            ...
            if dynamic:
                ...
                if (type(ptb) != PerturbationLpNormWithRegion or ptb.norm < np.inf
