    ...
    def forward(self, *inputs, **kwargs):
        ...
        if len(inputs) > 0 and hasattr(inputs[0], 'ptb') and inputs[0].ptb is not None:
            ...
            for input_s in inputs_scatter:  # GPU numbers
                ...
                assert isinstance(inputs[0].ptb, PerturbationLpNormWithRegion)
                ptb = PerturbationLpNormWithRegion(norm=inputs[0].ptb.norm, eps=inputs[0].ptb.eps, x_L=input_s[1], x_U=input_s[2])
            ...
            for input_s, kw_s in zip(inputs_scatter, kwargs):  # GPU numbers
                # FIXME other perturbations are not supported yet
                assert isinstance(x.ptb, PerturbationLpNormWithRegion)
                ptb = PerturbationLpNormWithRegion(norm=x.ptb.norm, eps=x.ptb.eps, x_L=input_s[1], x_U=input_s[2])
                kw_s['x'] = list(kw_s['x'])
